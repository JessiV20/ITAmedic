...
cambiarEstadoCita(idcita: number, nuevoEstado: 3 | 1): void {
  ...
  get citasEspera(): number {
    return this.citas.filter(c => c.estatus === 2).length;
  }

  get citasAtendidas(): number {
    return this.citas.filter(c => c.estatus === 3).length;
  }

  get citasCanceladas(): number {
    return this.citas.filter(c => c.estatus === 1).length;
  }
 ...
 loadCitas(): void {
  ...
  private normalizarEstatus(estatus: number): 2 | 3 | 1 {
    const estatusLower = estatus;
    if (estatusLower === 3) return 3;
    if (estatusLower === 1) return 1;
    return 2;
  }
